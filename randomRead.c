...
{
	if(argc!=4){
		printf("Requires 3 arguments: <devicePath> <minReadSize> <maxReadSize>\n");
		return 1;
	}

	//Parse args
	int minReadSizeBytes, maxReadSizeBytes;
	char *devicePath = argv[1];
	sscanf(argv[2], "%d", &(minReadSizeBytes));
        sscanf(argv[3], "%d", &(maxReadSizeBytes));
	...
	int minReadSizeSectors = getNumSectors(minReadSizeBytes, sectorSizeBytes);
       	int maxReadSizeSectors = getNumSectors(maxReadSizeBytes, sectorSizeBytes);
	int maxReadSizeDiffSectors = maxReadSizeSectors - minReadSizeSectors;
	...
	char *readBuffer = memalign(pageSize, maxReadSizeSectors * sectorSizeBytes);

	//Do the ops
	while(1==1){
		long numSectorsToRead = getUnbiasedRandomLong(maxReadSizeDiffSectors) + minReadSizeSectors;
		long bytesToRead = numSectorsToRead * sectorSizeBytes;
		long sectorToSeekTo = getUnbiasedRandomLong(numDeviceSectors - numSectorsToRead);
